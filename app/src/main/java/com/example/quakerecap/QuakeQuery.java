...
public final class QuakeQuery {
    ...
    public static final class QueryClass {
        ...
        // RETOURNE UN ARRAY LIST DE TYPE POJO CLASS AVEC TROIS PARAMETRES
        public static ArrayList<PojoClass> extractEarthquakes() {

            // LE TABLEAU QUI VA STOCKER LES SCENARIO A VENIR QUI EST A RETOURNER
            ArrayList<PojoClass> quakes_data = new ArrayList<>();
            ...
            try {
                ...
                // N'OUBLIE PAS QU'ON S'EST REFERÉ AU SITE DU JSON FORMATER POUR EXPLOITER CE JSON ROOT
                // POINTER SUR LE TOUT DES JSON OBJETS DE LA REQUETTE EN LA CONVERTISSANT EN U JSON OBJET
                JSONObject root=new JSONObject(SAMPLE_JSON_RESPONSE);
                // POINTER SUR LA JSON ARRAY DE NOM "FEATURES" DANS LE ROOT
                //REGARDE COMMENT LES CHOSES SE DESSINENT !!!
                //CREATION DU ROOT, ROOT POINT GET JSON ARRAY, JSON ARRAY POINT GET JSON OBJET, JSON OBJET POINT GET STRING....
                JSONArray earthquakeArray=root.getJSONArray("features");

                // PARCOURRIR LA JSON ARRAY
                for (int i=0; i <earthquakeArray.length();i++){

                    //POINTER SUR CHAQUE INDICE(CE SONT DES JSON OBJETS) DE LA JSON ARRAY POUR EXPLORER
                    JSONObject currentquake=earthquakeArray.getJSONObject(i);

                    // POINTER SUR LA JSON OBJET DE NOM "PROPERTIES" DANS CHAQUE INDICE DE LA JSON ARRAY
                    JSONObject properties =currentquake.getJSONObject("properties");

                    //POINTER SUR LE STRING "MAG" DANS LA JSON OBJET "PROPERTIES"
                    double mag= properties.getDouble("mag");

                    // POINTER SUR LE STRING "PLACE" DE LA JSON OBJET PROPERTIES
                    String place=properties.getString("place");

                    //POINTER SUR LE STRING "TIME" DE LA JSON OBJET PROPERTIES
                    // ET CONVERTIR LE TIME EN LONG CAR IL EST TROP LONG
                    long time=properties.getLong("time");
                    String url=properties.getString("url");


                    // CONVERTIR LE TIME CONVERTI EN LONG EN UNE DATE DE FORMAT "2016-05-13 12:07:46 PM"
                    //POUR LA PETITE HISTOIRE, LES CONVERSION DE LONG EN DATE SONT DEJA PROGRAMMÉES PAR LES EXPERTS
                    quakes_data.add(new PojoClass(mag, place, time,url));
                ...
                Log.e("QueryClass", "Problem parsing the earthquake JSON results", e);
            }
